...
public class PollExportController {
    ...
    private static final String PERMISSION_EDIT_ANY = "poll.editAny";
    private static final String PERMISSION_EDIT_OWN = "poll.editOwn";

    @GetMapping("/polls/export/xlsx/{pollId}")
    public ResponseEntity<?> exportXlsx(@PathVariable("pollId") String pollId, 
                                        Locale locale, 
        ...
        try (Workbook wb = new XSSFWorkbook(); ByteArrayOutputStream bos = new ByteArrayOutputStream()) {
            ...
            String safePollText = poll.getText().replaceAll("[^a-zA-Z0-9-_]", "_");
            if (safePollText.length() > 30) {
                safePollText = safePollText.substring(0, 30);
            }

            String timestamp = LocalDateTime.now().format(DateTimeFormatter.ofPattern("yyyyMMdd_HHmm"));
            String filename = "Poll_" + safePollText + "_" + timestamp + ".xlsx";
            ...
            redirectAttributes.addFlashAttribute("alert", "Error interno al generar el reporte de Excel.");
            return ResponseEntity.status(HttpStatus.FOUND).header(HttpHeaders.LOCATION, "/votePolls").build();
        }
    }
